...
import (
	...
	semp "github.com/cjunior1/solace-semp-api-golang"
)

func main() {

	cfg := semp.NewConfiguration()
	client := semp.NewAPIClient(cfg)

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	// credenciais
	credencial := &semp.BasicAuth{}
	credencial.UserName = os.Getenv("SOLACE_USER_NAME")
	credencial.Password = os.Getenv("SOLACE_PASSWORD")

	ctx = context.WithValue(ctx, semp.ContextBasicAuth, *credencial)
	...
	if err != nil {
		...
		novoTopic := semp.MsgVpnQueueSubscription{MsgVpnName: serviceName, QueueName: queueName, SubscriptionTopic: subscriptionTopic}
